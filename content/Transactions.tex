...
\begin{description}%function get_pre_mlsag_hash calculates the message signed
	...
	To\marginnote{src/ringct/ rctSigs.cpp prove- RangeBulletproof()} avoid the amount ambiguity of outputs described in Section  \ref{sec:range_proofs}, the sender must employ the Bulletproofs proving system to assert amount ranges for each output $t \in \{1, ..., p\}$. A single aggregate Bulletproof is used for all outputs in a transaction. \\
...
From\marginnote{src/ringct/ rctSigs.cpp prove- RangeBulletproof()} Section \ref{sec:range_proofs} and Section \ref{full-signature}, we know that an aggregate range proof on $p$ outputs requires \( ( 2 \lceil \log_2(64p) \rceil + 9 ) 32 \) bytes total.
