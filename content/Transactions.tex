...
Since this value can be recomputed by the recipient, it is not included with the transaction data.

Commitment amounts are computed differently, since they are not generated uniformly. Each commitment value scalar occupies $32$ bytes, but amounts are restricted to be $8$ bytes for range proof purposes. To save space, amounts are masked as follows and included in the transaction data:\marginnote{src/ringct/ rctOps.cpp ecdhEch- code()}\\
